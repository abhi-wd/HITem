...
function Game() {
  ...
  const [grid, setGrid] = useState<(null | { type: string; icon: string })[]>(
    Array(9).fill(null)
  );
  ...
  useEffect(() => {
    ...
    const spawnInterval = setInterval(spawn, 1200); // fixed speed
    return () => clearInterval(spawnInterval);
  }, [gameStarted, gameOver]);

  // Handle clicks
  const handleCellClick = (
    cell: { type: string; icon: string } | null,
    e: React.MouseEvent
  ) => {
    if (!cell) return;
    if (cell.type === "coin") {
  ...
  const handleClaimReward = () => {
    alert(`Reward claimed! Sending ${tokens} ARB tokens to your wallet...`);
  };
        ...
        <>
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="px-6 py-3 rounded-2xl text-lg bg-green-400 text-black hover:bg-green-500"
            >
              Replay 🔄
            </button>
          </div>
